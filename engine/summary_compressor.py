...
主路径接通点:
- agent/runtime/agent_loop.py: 每轮 verify 失败时, 把 AgentFeedback 压到 1200
  字符再 inject 进下一轮 user message。
- prover/unified/tools_extra.py::BroadcastTool: ``broadcast_publish`` 操作
  的 ``content`` 字段先压再发, 防止 4 路 sub-profile 互相塞爆。
...
Design constraints:
  ...
  - Hard budget on output chars (default 1200)
  - Dedup identical/near-identical error lines
  - Preserve error category distribution for diagnosis

Usage::

    from engine.summary_compressor import compress_lean_errors, compress_feedback
