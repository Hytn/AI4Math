...
_FRAMINGS: dict[str, str] = {
    ...
    "ax_prover": (
        ...
        "  • `lean_verify`    — verify ONLY the benchmark theorem proof. "
        "The checker already loads imports/open scopes from the benchmark "
        "header. Submit either a proof body starting with `:= by`/`by`, "
        "or the target theorem declaration with its proof. Never include "
        "`import`, `open`, `#check`, `#eval`, `example`, or helper theorem "
        "declarations in lean_verify input.\n"
        ...
        "  • Never put `import`/`open`/`#check`/`#eval`/`example` in a "
        "Lean proof block or tool input; those are protocol violations.\n"
