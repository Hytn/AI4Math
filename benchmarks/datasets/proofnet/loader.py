"""benchmarks/datasets/proofnet/loader.py — proofnet dataset loader"""
...
def load(repo_path: str, split: str = "test") -> list[BenchmarkProblem]:
    """Load proofnet problems from local clone."""
    path = Path(repo_path)
    if not path.exists():
        logger.warning(f"proofnet path not found: {repo_path}")
        return []
    ...
    if manifest.exists():
        ...
        return [BenchmarkProblem(
            problem_id=item.get("problem_id", f"proofnet_{i}"),
    ...
    for lean_file in sorted(path.rglob("*.lean")):
        ...
        for m in re.finditer(r"^(theorem\s+(\S+).*?)(?=\n(?:theorem|lemma|def|end)\s|\Z)", content, re.MULTILINE | re.DOTALL):
            ...
            problems.append(BenchmarkProblem(
                problem_id=f"proofnet_{split}_{name}", name=name,
                theorem_statement=stmt, source="proofnet"))
    logger.info(f"Loaded {len(problems)} problems from proofnet")
    return problems
